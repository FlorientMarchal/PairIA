    ...
    <div class="catalog-top">
      ...
      <div class="filter-bar">
        <div class="dropdown" id="dd-categorie">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-categorie')">Catégorie <span class="dd-arrow">▾</span></button>
          <div class="dd-menu">
            <?php foreach ($categories as $cat): ?>
              <label class="dd-item"><input type="checkbox" data-filter="categorie" value="<?= htmlspecialchars($cat) ?>" onchange="applyFilters()"> <?= htmlspecialchars($cat) ?></label>
            <?php endforeach; ?>
          </div>
        </div>
        <div class="dropdown" id="dd-genre">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-genre')">Genre <span class="dd-arrow">▾</span></button>
          <div class="dd-menu">
            <?php foreach ($genres as $g): ?>
              <label class="dd-item"><input type="checkbox" data-filter="genre" value="<?= htmlspecialchars($g) ?>" onchange="applyFilters()"> <?= htmlspecialchars($g) ?></label>
            <?php endforeach; ?>
          </div>
        </div>
        <div class="dropdown" id="dd-marque">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-marque')">Marque <span class="dd-arrow">▾</span></button>
          <div class="dd-menu">
            <?php foreach ($marques as $m): ?>
              <label class="dd-item"><input type="checkbox" data-filter="marque" value="<?= htmlspecialchars($m) ?>" onchange="applyFilters()"> <?= htmlspecialchars($m) ?></label>
            <?php endforeach; ?>
          </div>
        </div>
        <div class="dropdown" id="dd-prix">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-prix')">Prix <span class="dd-arrow">▾</span></button>
          <div class="dd-menu dd-menu-prix">
            <div class="prix-label">Prix maximum : <strong id="prix-val"><?= $prix_max ?> €</strong></div>
            <input type="range" id="f-prix" min="0" max="<?= $prix_max ?>" value="<?= $prix_max ?>" step="5" oninput="updatePrix(this.value)">
            <div class="prix-range-labels"><span>0 €</span><span><?= $prix_max ?> €</span></div>
          </div>
        </div>
        <div class="dropdown" id="dd-taille">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-taille')">Pointure <span class="dd-arrow">▾</span></button>
          <div class="dd-menu">
            <?php foreach ($tailles as $t): ?>
              <label class="dd-item"><input type="checkbox" data-filter="taille" value="<?= $t ?>" onchange="applyFilters()"> <?= $t ?></label>
            <?php endforeach; ?>
          </div>
        </div>
        <div class="dropdown" id="dd-couleur">
          <button class="dd-btn" type="button" onclick="toggleDropdown('dd-couleur')">Couleur <span class="dd-arrow">▾</span></button>
          <div class="dd-menu">
            <?php foreach ($couleurs as $c): ?>
              <label class="dd-item"><input type="checkbox" data-filter="couleur" value="<?= htmlspecialchars($c) ?>" onchange="applyFilters()"> <?= htmlspecialchars($c) ?></label>
            <?php endforeach; ?>
          </div>
        </div>
        <button class="filter-reset-btn" id="filter-reset-btn" type="button" onclick="resetFilters()">✕ Effacer</button>
      </div>
    </div>
          ...
          <a href="article.php?id=<?= $a['id_shoes'] ?>" class="card-link">
            <div class="card-img">
              <?php if (!empty($a['url_image'])): ?>
                <img src="<?= htmlspecialchars($a['url_image']) ?>" alt="<?= htmlspecialchars($a['nom']) ?>" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
...
function updateDropdownState(id, count) {
  ...
  if (!btn) return;
  btn.style.background  = count > 0 ? 'var(--accent)' : '';
  btn.style.color       = count > 0 ? 'white' : '';
  btn.style.borderColor = count > 0 ? 'var(--accent)' : '';
}
function updatePrix(val) {
  ...
  btn.style.background  = prixMax < prixMaxAbsolu ? 'var(--accent)' : '';
  btn.style.color       = prixMax < prixMaxAbsolu ? 'white' : '';
  btn.style.borderColor = prixMax < prixMaxAbsolu ? 'var(--accent)' : '';
  applyFilters();
}
function getChecked(filter) {
  return [...document.querySelectorAll(`input[data-filter="${filter}"]:checked`)].map(i => i.value);
}
function applyFilters() {
  const cats = getChecked('categorie'), genres = getChecked('genre'),
        marques = getChecked('marque'), tailles = getChecked('taille'), couleurs = getChecked('couleur');
  ...
  const hasFilters = cats.length || genres.length || marques.length || tailles.length || couleurs.length || prixMax < prixMaxAbsolu;
  ...
  document.querySelectorAll('.card').forEach(card => {
    ...
    const ok = (!cats.length || cats.includes(card.dataset.categorie)) &&
...
function resetFilters() {
  ...
  resetBtn.style.color = 'var(--gray)'; resetBtn.style.borderColor = 'var(--border)';
