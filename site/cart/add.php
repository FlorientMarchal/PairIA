...
$blacklist = [
    ...
    'va te faire','ta gueule','ferme ta gueule','ntm',
...
if ($apiKey) {
    $modPrompt = "Tu es un modérateur pour PairIA, boutique de chaussures haut de gamme.
...
- Contenu totalement hors-sujet sans aucun lien avec les chaussures, la mode, la livraison ou l'expérience d'achat (ex : discours politique, propagande, contenu personnel sans rapport)

IMPORTANT : Un avis sur la livraison, le service client, la pointure, le confort, le style ou la qualité est TOUJOURS accepté même s'il ne mentionne pas explicitement le mot \"chaussure\".
    ...
    if ($response && $httpCode === 200) {
        ...
        if (isset($parsed['ok']) && $parsed['ok'] === false) {
            echo json_encode([
                'success' => false,
                'error'   => 'contenu_inapproprie',
                'message' => $parsed['raison'] ?? 'Votre avis ne respecte pas notre charte. Merci de rester constructif et en lien avec votre expérience d\'achat.'
            ]);
            exit;
        }
    }
